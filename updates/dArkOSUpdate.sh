...
if [ ! -f "/home/ark/.config/.update????2025" ]; then
	...
	sudo rm -rf /dev/shm/*
	sudo wget -t 3 -T 60 --no-check-certificate "$LOCATION"/updates/????2025/arkosupdate????2025.zip -O /dev/shm/arkosupdate????2025.zip -a "$LOG_FILE" || sudo rm -f /dev/shm/arkosupdate????2025.zip | tee -a "$LOG_FILE"
